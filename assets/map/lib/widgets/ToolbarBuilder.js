 ...
 */
Ext.namespace("Ostim.widgets.ToolbarBuilder");

/** api: (define)
 *  module = Ostim.widgets
 ...
 * - a create function that returns an GeoExt, Ostim or Ext Component
 *
 * Within a Ostim config the Toolbar can be defined by adding definitions
 ...
 *
 * See the Ostim examples configs.
 * */
Ostim.widgets.ToolbarBuilder.defs = {
    ...
    zeynelToolbarItem: {
        ...
        create: function (mapPanel, options) {
            ...
            //console.warn('test ayraççç');
            //console.warn(options.getfeatureControl);
            if (options.getfeatureControl) {
                //console.log(options.getfeatureControl);
            ...
            if (options.popupWindow) {
                ...
                var createPopupWindow = function () {
                    ...
                    if (!self.featurePopupWindowZeynel) {
                        self.featurePopupWindowZeynel = new Ostim.widgets.search.ZeynelInfoPopup(popupWindowProps);
                        console.warn(self.featurePopupWindowZeynel);
                        console.warn('zeynelToolbarItem create popup');
                    }
                };
                ...
                options.handler = function () {
                    createPopupWindow();
                    self.featurePopupWindowZeynel.hide();
                };
            }
    ...
    baselayer: {
        ...
        create: function (mapPanel, options) {
            ...
            return new Ostim.widgets.BaseLayerCombo(options);
        }
    },
    ...
    geocoder: {
        ...
        create: function (mapPanel, options) {
            return new Ostim.widgets.search.GeocoderCombo(options);
        }
    },
    ...
    scale: {
        ...
        create: function (mapPanel, options) {
            ...
            Ext.apply(options.initialConfig, options);
            return new Ostim.widgets.ScaleSelectorCombo(options);
        }
    },
            ...
            if (options.popupWindow) {
                ...
                var createPopupWindow = function () {
                    ...
                    if (!self.featurePopupWindow) {
                        self.featurePopupWindow = new Ostim.widgets.search.FeatureInfoPopup(popupWindowProps);
                        console.warn(self.featurePopupWindow);
                        console.warn('feature info create popup');
                    }
                };
...
//                layout: 'fit',
    ...
    tooltips: {
        ...
        create: function (mapPanel, options) {
            ...
            return Ostim.widgets.ToolbarBuilder.defs.featureinfo.create(mapPanel, options);
        }
    },
    ...
    measurelength: {
        ...
        create: function (mapPanel, options) {
            ...
            Ostim.widgets.ToolbarBuilder.onMeasurementsActivate = function (event) {
                Ext.getCmp("measurelength").measureLastLength = 0.0;
            };

            Ostim.widgets.ToolbarBuilder.onMeasurements = function (event) {
            ...
            Ostim.widgets.ToolbarBuilder.onMeasurementsPartial = function (event) {
                var units = event.units;
                var measure = event.measure;
                var out = "";
                if (event.order == 1) {
            ...
            Ostim.widgets.ToolbarBuilder.onMeasurementsDeactivate = function (event) {
            ...
            for (var i = 0; i < controls.length; i++) {
                ...
                if (controls[i].displayClass == 'olControlMeasureDistance') {
                    ...
                    controls[i].events.register("activate", map, Ostim.widgets.ToolbarBuilder.onMeasurementsActivate);
                    controls[i].events.register("measure", map, Ostim.widgets.ToolbarBuilder.onMeasurements);
                    controls[i].events.register("measurepartial", map, Ostim.widgets.ToolbarBuilder.onMeasurementsPartial);
                    controls[i].events.register("deactivate", map, Ostim.widgets.ToolbarBuilder.onMeasurementsDeactivate);
                    break;
                }
            }
     ...
     * and enhanced for the Ostim project by Just: https://github.com/justb4/ole.
     *
     * */
    oleditor: {
    ...
    search_nominatim: {
        ...
        create: function (mapPanel, options) {
            return new Ostim.widgets.search.NominatimSearchCombo(options);
        }
    },
...
//            toggleGroup: "toolGroup",
    ...
    printdialog: {
        ...
        create: function (mapPanel, options) {
            ...
            options.handler = function () {
                ...
                if (!Ostim.widgets.ToolbarBuilder.checkCanWePrint(mapPanel.getMap().layers)) {
                    return;
                }

                var printWindow = new Ostim.widgets.PrintPreviewWindow({
            ...
            options.handler = function () {
                ...
                Ext.Ajax.request({
                    ...
                    success: function (result, request) {
                        ...
                        // See doc for 'Ostim.widgets.LayerLegendPanel'
                        if (options.mapPrintLegend) {
                            // Hidden LegendPanel : needed to fetch active legends
                            var legendPanel = new Ostim.widgets.LayerLegendPanel({
    ...
    coordinatesearch: {
        ...
        create: function (mapPanel, options) {
            ...
            options.handler = function () {
                ...
                if (!this.coordPopup) {
                    // default entries
                    var sp = new Ostim.widgets.search.CoordSearchPanel({ });
                    this.coordPopup = new Ext.Window({
                        ...
                        items: [new Ostim.widgets.search.CoordSearchPanel({
    ...
    addbookmark: {
        ...
        create: function (mapPanel, options) {
            ...
            options.handler = function () {
                var bookmarksPanel = Ostim.widgets.Bookmarks.getBookmarksPanel(this);
                if (!bookmarksPanel) {
    ...
    mapopen: {
        ...
        create: function (mapPanel, options) {
            options.handler = function () {
                Ostim.data.MapContext.openContext(mapPanel, options);
            }
            return new GeoExt.Action(options);
        }
    },
    mapsave: {
        ...
        create: function (mapPanel, options) {
            options.handler = function () {
                Ostim.data.MapContext.saveContext(mapPanel, options);
            }
            return new GeoExt.Action(options);
        }
    },
 ...
 */
Ostim.widgets.ToolbarBuilder.checkCanWePrint = function (layers) {
...
Ostim.widgets.ToolbarBuilder.setItemDef = function (type, createFun, defaultOptions) {
    Ostim.widgets.ToolbarBuilder.defs[type].create = createFun;
    Ostim.widgets.ToolbarBuilder.defs[type].options = defaultOptions ? defaultOptions : {};
};
 ...
 */
Ostim.widgets.ToolbarBuilder.build = function (mapPanel, config, toolbar) {
    //console.warn('Ostim.widgets.ToolbarBuilder.build');
    var toolbarItems = [];
    if (typeof(config) !== "undefined") {
        for (var i = 0; i < config.length; i++) {
            ...
            var createFun;
            var defaultItemDef = Ostim.widgets.ToolbarBuilder.defs[itemDef.type];
            if (itemDef.create) {
