...
/** api: example
 *  Sample code showing how to configure a Ostim FeatureInfoPanel.
 ...
 *							attrName: 'City',
 *							renderer: {
 *								fn : Ostim.widgets.GridCellRenderer.directLink,
 *								options : {
 ...
 *							attrName : 'Country',
 *							renderer :  {
 *								fn : Ostim.widgets.GridCellRenderer.browserPopupLink,
 *								options : {
 ...
 *							attrName : 'longitude',
 *							renderer :  {
 *								fn : Ostim.widgets.GridCellRenderer.valueSubstitutor,
 *								options : {
 ...
 *                      return Ostim.widgets.GridCellRenderer.substituteAttrValues(template, options, record);
 *                  }
 *            }
 *
 *           ]
 ...
 *
 */
Ostim.widgets.search.ZeynelInfoPanel = Ext.extend(Ext.Panel, {
    

    /**
     * zeynel dağlı
     * bu değişken harita üzerinde çok sayıda layer ve bu layerlara bağlı feature'lar
     * olduğu durumda featureset oluştururken 'layer' type değişkenine göre işlem yapılmasını
     * sağlamak için eklenmiştir
     * @since 05.09.2014
     */
    featureTypeCondition : null,
    
    /**
     * zeynel dağlı
     * harita ile etkileşimli olarak gösterilecek feature sayısının kısıtlanması
     * istendiğinde kullanılacak değişken. Değişken maksiumum feature saysyısına göre 
     * düzenlenmiştir. yani mesela değişken değeri '1' ise feature.lenght>1 
     * olduğunda koşul gerçekleşmiş olacaktır.
     * @since 05.09.2014
     */
    featureCountCondition : null,
     ...
     * ``ostim.cgi`` to be installed. Exporting results in a download of a document with the contents of the (Grid) Panel.
    ...
    initComponent: function () {
        ...
        Ostim.widgets.search.ZeynelInfoPanel.superclass.initComponent.call(this);
        ...
        console.warn('WMSGetFeatureInfo control sayısı-->'+controls.length);
        ...
        if (!this.olControl) {
            var controls = this.map.getControlsByClass("OpenLayers.Control.WMSGetFeatureInfo");
            //console.warn('WMSGetFeatureInfo control sayısı-->'+controls.length);
            console.warn('controls length ZeynelInfoPanel-->'+controls.length);
            if (controls && controls.length > 0) {
                for (var index = 0; index < controls.length; index++) {
                    //Control should not be the one used for tooltips.
                    console.warn('controls length if dışı -->'+index);
                    console.warn('control id -->'+controls[index].id);
                    if (controls[index].id !== "hr-feature-info-hover" /*&& controls[index].id == "zeynelinfo"*/) {
                        console.warn('controls length hr-feature-info-hover-->'+index);
    ...
    handleBeforeGetFeatureInfo: function (evt) {
        console.warn('handleBeforeGetFeatureInfo ZeynelInfoPanel');
    ...
    handleGetFeatureInfo: function (evt) {
        console.warn('handleGetFeatureInfo Function ZeynelInfoPanel');
        // Always restore possible temporary vendorParams
        
        var layers = this.olControl.layers;
        //console.warn('handleGetFeatureInfo');
        //console.warn(layers);
        if (layers) {
    ...
    handleVectorFeatureInfo: function (evt) {
        ...
        console.warn('handleVectorFeatureInfo function ZeynelInfoPanel');
        ...
        //console.warn(evt.features);
        if (evt.features && evt.features.length > 0) {
            //console.warn('evt.features length>0 test');
    ...
    getFeaturesByXY: function(x, y) {
        ...
        for (i=this.map.layers.length-1; i>=0; --i) {
            ...
            //console.warn(layer);
            //
            //console.warn('layer.div.style.display öncesi ');
            if (layer.div.style.display !== "none") {
                //console.warn('layer name-->'+layer.name);
                //console.warn('layer instanceof OpenLayers Test');
                if (layer instanceof OpenLayers.Layer.Vector) {
                    console.warn('layer instanceof OpenLayers');
                ...
                //console.warn('layer instanceof OpenLayers sonrası zeynel info');
                layers.push(layer);
    ...
    displayFeatures: function (evt) {
        console.warn('displayFeatures Function---->');
    ...
    /**
     * zeynel dağlı
     * harita ile etkileşimli olacak feature bilgileri
     * feature sayısına göre sınırlama gerektirirse kontrol
     * bu fonksiyonda yapılacaktır
     * @since 05.09.2014
     */
     lookForFeatureCount: function(featureSets, featureSetKey) {
         //console.warn(featureSets);
         var featureSet;
         var numerator=0;
         for (featureSetKey in featureSets) {
            featureSet = featureSets[featureSetKey];
            //console.warn('featureSets.length--->'+featureSet.features.length);
            numerator += featureSet.features.length;
         }
         console.warn('toplam feature count--->'+numerator);
    ...
    displayFeatureInfo: function (evt) {
        ...
        //console.log('features length-->'+evt.features.length);
        ...
        for (var index = 0; index < evt.features.length; index++) {
            ...
            /**
             * zeynel dağlı
             * feature type koşulu var ve koşul sağlanmıyorsa bu blokta
             * çalışacaktır. dolayısıyla 'featureset' içerisine ekleme yapmayacaktır.
             */
            if (this.featureTypeCondition!=null && this.featureTypeCondition!=featureType) {
                    continue;
            /**
             * zeynel dağlı
             * feature type koşulu yoksa yada koşul var ve koşul sağlanıyorsa bu blokta
             * çalışacaktır
             */
            ...
            /**
             * zeynel dağlı
             * haritada seçilem featureların depolandığı array
             */
            //console.warn(featureSet.features);
        }
         ...
         * zeynel dağlı
         */
        this.lookForFeatureCount(featureSets);
        ...
        for (featureSetKey in featureSets) {
            ...
            //console.warn('store set edildi');
             ...
             var panel = new Ostim.widgets.search.ZeynelFeaturePanel({
                 /**
                  * zeynel dağlı
                  * haritada seçilen feature'lar 'featurePanel'
                  * nesnesine aktarılıyor
                  * @since 18-12-2014
                  */
                featuresArray : featureSet.features,
    ...
    displayTree: function (evt) {
        var panel = new Ostim.widgets.XMLTreePanel();
    ...
    displayXML: function (evt) {
        var opts = {
            html: '<div class="hr-html-panel-body"><pre>' + Ostim.Utils.formatXml(evt.text, true) + '</pre></div>',
...
Ext.reg('hr_zeynelinfopanel', Ostim.widgets.search.ZeynelInfoPanel);
